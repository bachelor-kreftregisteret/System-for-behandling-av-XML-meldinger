    ...
    function ShowData() {
        if (!loading) {
            ...
            return msgList.map((item, index) =>
                <li key={index}>
                    {item[0].includes('Utredning') &&
                        <Link to={"utredning/" + item[1]}>{item[0]} </Link> }
                    {item[0].includes('Straalebehandling') &&
                        <Link to={"straalebehandling/" + item[1]}>{item[0]} </Link> }
                    {item[0].includes('Kirurgi') &&
                        <Link to={"kirurgi/" + item[1]}>{item[0]} </Link> }
                </li>)
        } else {
